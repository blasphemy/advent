...
const challengeInput = `62 1649 1731 76 51 1295 349 719 52 1984 2015 2171 981 1809 181 1715
...
func main() {
	log.Println(getNumber(challengeInput))
}
...
func getNumber(input string) int {
	...
	for _, x := range rows {
		...
		big, small := getBiggestAndSmallest(nums)
		diff := big - small
		sum = sum + diff
	}
	return sum
...
func getBiggestAndSmallest(i []int) (int, int) {
	big := 0
	for _, y := range i {
		if y > big {
			big = y
